...
orbitRepresentatives(Ring, VisibleList, VisibleList) := List => o -> (R, prev, degs) -> (
    -- Pass not an ideal, but a list of ideals. This should allow for much reduced computation time.
    G := permutations R;
    result := prev;
    rawMonsMat := matrix{{}};
    mons := {};
    for d in degs do (
        rawMonsMat = monomialsInDegree(d, R, o.MonomialType);
        time mons = flatten entries sort(rawMonsMat, 
                     DegreeOrder => Ascending, MonomialOrder => Descending);
        result = normalForms(sumMonomials(result, mons), G)
        );
    result
